...
if __name__ == "__main__":
    ...
    for reb in sorted(fpdict):
        if fpdict[reb]["HVstate"] and fpdict[reb]["enable"]:
            print("{} has BackBias ON -- turning BackBias off".format(reb))
            getattr(fp, reb)().setBackBias(False)
            print("getattr(fp, {})().setBackBias('False')".format(reb))
            time.sleep(seqdelay)
        else:
